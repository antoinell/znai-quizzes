...
function registerZnaiCounterComponent() {
  // Wait for znai to be available
  const checkAndRegister = () => {
    console.log('in checkandregister');
    if (window.znai?.elementsLibrary?.library) {
      // Register the ZnaiQuiz component
      window.znai.elementsLibrary.library.ZnaiCounter = ZnaiCounter;
      console.log('ZnaiCounter component registered successfully');
    } else {
